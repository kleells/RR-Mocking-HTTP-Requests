...
beforeEach(() => {
  ...
  describe('receives values from GitHub REST API using jest fetch mock', () => {
    ...
    test("receives GitHub URL", async () => {
      fetch.mockResponseOnce(JSON.stringify({html_url: 'https://api.github.com/users/kleells'}))
      render(<App />)
    ...
    test("receives GitHub Image URL", async () => {
      fetch.mockResponseOnce(JSON.stringify({avatar_url: 'https://avatars.githubusercontent.com/u/97200567?v=4'}))
      render(<App />)
